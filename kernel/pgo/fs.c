...
static const struct proc_ops prf_fops = {
	.proc_open		= prf_open,
	.proc_read		= prf_read,
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static const struct proc_ops prf_reset_fops = {
...
static int __init pgo_init(void)
{
	...
	if (!debugfs_create_file("vmlinux.profraw", 0444, directory, NULL,
				 &prf_fops))
		goto err_remove;

	if (!debugfs_create_file("reset", 0222, directory, NULL,
				 &prf_reset_fops))
		goto err_remove;
#else
	directory = proc_mkdir("pgo", NULL);
        ...
        if (!proc_create("vmlinux.profraw", 0444, directory, &prf_fops))
                goto err_remove;

        if (!proc_create("reset", 0222, directory, &prf_reset_fops))
                goto err_remove;
#endif
	return 0;
